...
with tabs[0]:
    ...
    with st.form("mzML-upload", clear_on_submit=True):
        ...
        if cols[1].form_submit_button(
            "Add mzML/raw file to workspace",
            type="primary",
        ):
            if not files:
    ...
    if any(Path(mzML_dir).iterdir()):
        ...
        df = pd.DataFrame(
            {
                "file name": [
                    item
                    for item in file_names_
                    if not (item.endswith(".csv") or item.endswith(".mgf"))
                ]
            }
        )
        ...
        with st.expander("🗑️ Remove uploaded mzML/raw files"):
            ...
            if c1.button(
                "⚠️ Remove **all**",
                disabled=not any(mzML_dir.iterdir()),
            ):
...
with tabs[1]:
    ...
    if any(Path(fasta_dir).iterdir()):
        ...
        with st.expander("🗑️ Remove uploaded fasta files"):
            ...
            # Remove selected files
            if c2.button(
                "Remove **selected** from workspace",
                type="primary",
                disabled=not any(to_remove),
            ):
            ...
            # Remove all files
            if c1.button(
                "⚠️ Remove **all** from workspace",
                disabled=not any(fasta_dir.iterdir()),
            ):
...
with tabs[2]:
    ...
    def function_to_load_example_data():
        ...
        zip_url = (
            "https://github.com/Arslan-Siraj/NuXL_rescore_resources/releases/"
            "download/0.0.3/RNA_DEB_Protein_NA_XL_example_files.zip"
        )
    ...
    if not any(Path(example_data_dir).iterdir()):
        ...
        if any(Path(example_data_dir).iterdir()):
            ...
            # Display all example files currently in workspace
            file_names_ = [f.name for f in Path(example_data_dir).iterdir()]
            df = pd.DataFrame(
                {
                    "file name": [
                        item for item in file_names_ if not item.endswith(".txt")
                    ]
                }
            )
            ...
            if st.button(
                "Reload example data files to workspace",
                type="primary",
                help="This will overwrite existing example files in the workspace.",
            ):
                function_to_load_example_data()
